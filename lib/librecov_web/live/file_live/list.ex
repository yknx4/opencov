          ...
          <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-ecom-filters" style="">
            {#for {k, v} <- raw_filters()}
              {#if Enum.any?(filters(), &(&1 == k))}
                <Link class="dropdown-item d-flex align-items-center justify-content-between" label={v} to={apply(@path_fn, @path_args ++ [[{:filters, filters() -- [k]}|order_args]])} />
              {#else}
                <Link class="dropdown-item d-flex align-items-center justify-content-between" label={v} to={apply(@path_fn, @path_args ++ [[{:filters, [k|filters()]}|order_args]])}  />
              {/if}
            {/for}
          </div>
        </div>
            ...
            <tr>
              {#for {k, v} <- %{"coverage" => "Coverage", "diff" => "Diff", "name" => "Name"}}
                <th class="d-none d-sm-table-cell text-center">
                  <Link to={apply(@path_fn, @path_args ++ [[filters: filters(), order_field: k, order_direction: row_order(@order, k)]])}>
                    <span>{ v }</span>
